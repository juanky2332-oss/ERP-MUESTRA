...
export function useDeliveryNotes({
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            const totalEntregado = statsData?.reduce((acc, curr) => acc + (curr.total || 0), 0) || 0

            const totalPendiente = statsData?.reduce((acc, curr) => {
                const isTraspasado = curr.statuses?.includes('traspasado')
                if (!isTraspasado) return acc + (curr.total || 0)
                return acc
            }, 0) || 0
    ...
    const stats = data?.stats || { totalEntregado: 0, totalPendiente: 0 }
    ...
    // Mutations for create/update would go here if needed...
    const updateDeliveryNote = useMutation({
        mutationFn: async ({ id, ...updates }: Partial<Albaran> & { id: string }) => {
            ...
            const result = await updateDocument(id, updates, 'albaran')
